...
const createRazorpayOrder = async (req, res) => {
  try {
    const { bookingId, propertyId, serviceId, amount, currency = 'INR', pricingToken, pricingContext, addonServicesTotal = 0 } = req.body;
    ...
    if (bookingId) {
      ...
      // The token was generated by pricing.controller.js with the DISCOUNTED total
      // (property total including discount, GST, processing fee). Addon services
      // are NOT part of the token — they are validated during booking creation.
      // ═══════════════════════════════════════════════════════════════════════════
      const frontendTotal = Number(pricingContext.totalAmount); // includes addons
      const addonTotal = Number(addonServicesTotal || 0);
      const basePropertyTotal = frontendTotal - addonTotal; // token was generated for this
      ...
      try {
        ...
        isTokenValid = false;
        logger.warn('Token verification threw error', { error: error.message, tokenPayload });
      }

      if (!isTokenValid) {
        // Token verification is non-fatal for the order creation flow.
        // The amount will be fully re-verified during booking creation via
        // the centralized pricing engine. Log a warning for monitoring.
        logger.warn('Pricing token could not be verified (non-fatal)', {
          basePropertyTotal,
          frontendTotal,
          addonTotal,
          tokenError
        ...
        logger.info('Pricing token verified successfully', { basePropertyTotal, frontendTotal, addonTotal });
      }

      // Trust the frontend total for Razorpay order creation.
      // The full amount will be re-verified during booking creation by the
      // centralized pricing engine (services/pricing.service.js).
      finalAmount = frontendTotal;
      ...
      // Amount from the client body must match the pricing context total
      if (amount != null && Math.abs(Number(amount) - finalAmount) > 0.01) {
        return res.status(400).json({
          success: false,
